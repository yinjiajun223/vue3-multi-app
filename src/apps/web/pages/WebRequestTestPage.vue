...
async function handleLoadStats() {
  ...
  try {
    ...
    statsText.value = `访问量 ${data.visits}，今日新增 ${data.today}`
    appStore.markRequest('web 请求成功')
  }
  catch {
...
<template>
  <app-section title="Web 请求测试">
    <base-card title="axios 封装验证" description="调用 /public/mock/stats.json 并展示返回结果。">
